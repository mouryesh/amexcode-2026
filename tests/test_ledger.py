...
from backend import actions, database, ledger
from backend.database import db_session, init_db
from shared.exceptions import IdempotencyConflict


def _fresh_db():
    """Empty the ledger between tests.

    The append-only guard blocks DELETE — that is the whole point of it — so a
    test has to lift it explicitly. Application code has no way to.
    """
    init_db()
...
def test_clean_chain_verifies():
    _fresh_db()
    ledger.append("tool", "system", "reverse_fee", {"amount": 500}, {"outcome": "APPROVE"}, "s1")
    ledger.append("tool", "system", "block_card", {"card_id": "C1"}, None, "s1")
    ledger.append("tool", "system", "issue_replacement", {"card_id": "C1"}, None, "s1")
...
def test_tampered_chain_identifies_record():
    _fresh_db()
    ledger.append("tool", "system", "a", {"x": 1}, None, "s2")
    ledger.append("tool", "system", "b", {"x": 2}, None, "s2")
    ledger.append("tool", "system", "c", {"x": 3}, None, "s2")

    # Tamper with the second record's inputs directly in the DB. The
    # append-only guard has to be lifted to do it at all, which is the first
    # half of the proof; the chain catching it anyway is the second.
    with database.ledger_guard_disabled():
        with db_session() as conn:
...
def test_appends_chain_forward():
    _fresh_db()
    r1 = ledger.append("tool", "system", "a", {"x": 1}, None, "s3")
    r2 = ledger.append("tool", "system", "b", {"x": 2}, None, "s3")
    ...
    r3 = ledger.append("tool", "system", "a", {"x": 1}, None, "s3")
...
def _fresh_accounts_db():
    ...
    with db_session() as conn:
        ...
        conn.execute("DELETE FROM transactions WHERE member_id = 'MEM-CONC'")
        conn.execute("DELETE FROM accounts WHERE member_id = 'MEM-CONC'")
        # opened_at is NOT NULL now — account_age_days is derived from it.
        conn.execute(
            "INSERT INTO accounts (member_id, name, tenure_months, balance,"
            " credit_limit, opened_at)"
            " VALUES ('MEM-CONC', 'Concurrency', 24, 100000, 100000, '2024-01-01T00:00:00+00:00')"
        )
        # The fee transactions these tests reverse have to exist. reverse_fee
        # now reads the effect back, so reversing a txn_ref that was never
        # there raises EffectUnverified instead of silently succeeding.
        for ref in ("TXN-X", "TXN-A", "TXN-B"):
            conn.execute(
                "INSERT INTO transactions (txn_ref, member_id, date, amount,"
                " description, kind) VALUES (?, 'MEM-CONC', '2026-01-01', 500,"
...
def test_concurrent_appends_do_not_fork_the_chain():
    ...
    def worker(t: int):
        for i in range(per_thread):
            ledger.append("tool", "system", "concurrent", {"t": t, "i": i}, None, "sC")
