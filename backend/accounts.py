...
def get_unreversed_fees(member_id: str) -> list[dict[str, Any]]:
    """Every unreversed fee, not just the newest.

    Two matches means the graph asks which one; it must never pick silently.
    """
    with db_session() as conn:
        _require_member(conn, member_id)
        rows = conn.execute(
            """SELECT * FROM transactions
               WHERE member_id = %s AND kind = 'fee' AND reversed = FALSE
               ORDER BY date DESC""",
            (member_id,),
        ).fetchall()
        return [{**r, "amount": float(r["amount"])} for r in rows]
...
def get_account_facts(member_id: str, distress_signals: bool = False) -> dict[str, Any]:
    ...
    Every name a policy YAML references by `field` must appear here.
    """
    with db_session() as conn:
        row = _require_member(conn, member_id)
